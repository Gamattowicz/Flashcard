export const EXERCISE_CREATE_REQUEST = 'EXERCISE_CREATE_REQUEST'
export const EXERCISE_CREATE_SUCCESS = 'EXERCISE_CREATE_SUCCESS'
export const EXERCISE_CREATE_FAIL = 'EXERCISE_CREATE_FAIL'
export const EXERCISE_CREATE_RESET = 'EXERCISE_CREATE_RESET'

export const EXERCISE_ADD_CORRECT_ANSWER_REQUEST =
  'EXERCISE_ADD_CORRECT_ANSWER_REQUEST'
export const EXERCISE_ADD_CORRECT_ANSWER_SUCCESS =
  'EXERCISE_ADD_CORRECT_ANSWER_SUCCESS'
export const EXERCISE_ADD_CORRECT_ANSWER_FAIL =
  'EXERCISE_ADD_CORRECT_ANSWER_FAIL'

export const EXERCISE_ADD_WRONG_ANSWER_REQUEST =
  'EXERCISE_ADD_WRONG_ANSWER_REQUEST'
export const EXERCISE_ADD_WRONG_ANSWER_SUCCESS =
  'EXERCISE_ADD_WRONG_ANSWER_SUCCESS'
export const EXERCISE_ADD_WRONG_ANSWER_FAIL = 'EXERCISE_ADD_WRONG_ANSWER_FAIL'

export const EXERCISE_LIST_REQUEST = 'EXERCISE_LIST_REQUEST'
export const EXERCISE_LIST_SUCCESS = 'EXERCISE_LIST_SUCCESS'
export const EXERCISE_LIST_FAIL = 'EXERCISE_LIST_FAIL'

export const EXERCISE_ALL_LIST_REQUEST = 'EXERCISE_ALL_LIST_REQUEST'
export const EXERCISE_ALL_LIST_SUCCESS = 'EXERCISE_ALL_LIST_SUCCESS'
export const EXERCISE_ALL_LIST_FAIL = 'EXERCISE_ALL_LIST_FAIL'

export const EXERCISE_DETAILS_REQUEST = 'EXERCISE_DETAILS_REQUEST'
export const EXERCISE_DETAILS_SUCCESS = 'EXERCISE_DETAILS_SUCCESS'
export const EXERCISE_DETAILS_FAIL = 'EXERCISE_DETAILS_FAIL'

export const EXERCISE_UPDATE_TIME_REQUEST = 'EXERCISE_UPDATE_TIME_REQUEST'
export const EXERCISE_UPDATE_TIME_SUCCESS = 'EXERCISE_UPDATE_TIME_SUCCESS'
export const EXERCISE_UPDATE_TIME_FAIL = 'EXERCISE_UPDATE_TIME_FAIL'
